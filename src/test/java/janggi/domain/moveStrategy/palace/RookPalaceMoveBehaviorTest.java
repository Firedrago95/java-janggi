...
@ReplaceUnderBar
class RookPalaceMoveBehaviorTest {

    private RookPalaceMoveBehavior rookPalaceMoveBehavior = new RookPalaceMoveBehavior();
    ...
    void 궁성_경로를_반환한다(Position start, Position destination, List<Position> expected) {
        // when
        List<Position> path = rookPalaceMoveBehavior.getPath(start, destination);
